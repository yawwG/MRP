...
def get_parser():
    ...
    parser.add_argument(
        "--fold", type=int, default=0, help="fold of cross_validation"
    )

    parser.add_argument(
        "--cross_validation",  action="store_true", default=False
    )
...
def main(cfg, args):
    ...
    # get datamodule
    dm = src.builder.build_data_module(cfg,args)

    # define lightning module
    model = src.builder.build_lightning_model(cfg, dm)
    ...
    # )
    trainer = Trainer.from_argparse_args(
        args=trainer_args, deterministic=False, callbacks=callbacks, logger=logger
    )
    # trainer.check_test_every_n_epoch = 1
...
if __name__ == "__main__":
    ...
    for split in np.arange(args.splits):
        ...
        if args.cross_validation:
            cfg.output_dir = cfg.lightning.logger.save_dir + '/fold' + str(args.fold)+ '/'
            cfg.lightning.checkpoint_callback.dirpath =  cfg.lightning.logger.save_dir + '/fold' + str(args.fold)+ '/'
